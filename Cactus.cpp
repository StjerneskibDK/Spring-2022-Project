...
Cactus::Cactus() {
	...
	currentWaterLevel = 24;
	maxWaterLevel = 24;
	criticalWater = 3;
	lowerWater = 14;
	upperWater = 29;

	currentFertilizerLevel = 30;
	maxFertilizerLevel = 30;
	criticalFertilizer = 8;
	lowerFertilizer = 18;
	upperFertilizer = 36;
...
void Cactus::UserInput(bool clearToken) {
	...
	cout << "Enter one of the following input options, or S to exit and save the game" << endl << endl
		<< "W: Water cactus (+" << watering << " water) | F: Fertilize cactus (+4 fertilizer) | ";
	if (status_buginfest == true) { cout << "R: Remove bugs  |  "; }
	cout << "N: No input";
	...
	if (status_drought == true) {
		if (droughtCounter == 0) {
			...
			cout << "Drought. Days remaining: " << droughtCounter << " (inneffective)" << endl;
		}
	}
	...
	if (status_heatwave == true) {
		if (heatwaveCounter == 0) {
			...
			cout << "Heatwave. Days remaining: " << heatwaveCounter << " (ineffective)" << endl;
		}
	}
	if (status_coldsnap == true) {
		if (coldsnapCounter == 0) {
			...
			cout << "Cold snap. Days remaining: " << coldsnapCounter << " (DANGEROUS!)" << endl;
		}
	}
	if (status_buginfest == true) {
		cout << "Bug infestation. Remove the bugs to save your cactus!" << endl;
	}
	...
	if (input == 'W') {
		...
		cout << "Watered the cactus! (+" << watering << " water)" << endl << endl;
	}
	else if (input == 'w') {
		currentWaterLevel += watering;
		cout << "Watered the cactus! (+" << watering << " water)" << endl << endl;
		...
		cout << "Fertilized the cactus! (+" << fertilizing << " fertilizer)" << endl << endl;
	}
	else if (input == 'f') {
		currentFertilizerLevel += fertilizing;
		cout << "Fertilized the cactus! (+" << fertilizing << " fertilizer)" << endl << endl;
		...
		status_buginfest = false;
		cout << "Bug infestation removed!" << endl << endl;
	}
	else if (input == 'r') {
		status_buginfest = false;
		cout << "Bug infestation removed!" << endl << endl;
	...
	if (status_buginfest == true) {
		cout << "Bug infestation. Remove the bugs to save your plant!" << endl;
	}
...
void Cactus::NextDay() {
	...
	if (currentMaturity + 1 == maxMaturity) {
		cout << "Your cactus has reached maturity, great job!" << endl << endl;
		winToken = true;
		return;
	}
	currentMaturity += 1;
	...
	if (currentWaterLevel >= 21 && currentWaterLevel <= 24) {
	...
	if (currentFertilizerLevel >= 26 && currentFertilizerLevel <= 30) {
		currentEnergyLevel += 2;
	}
	...
	if (statusRandom <= 3 && status_drought != true) { status_drought = true; }
	statusRandom = rand() % 20;
	...
	statusRandom = rand() % 20;
	if (statusRandom <= 3 && status_buginfest != true) { status_buginfest = true; }
	if (health <= 0) {
		cout << "Your cactus has perished... better luck next time." << endl << endl;
		loseToken = true;	
		return;
	}
}
